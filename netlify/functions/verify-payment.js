...
export async function onRequestPost(context) {
  ...
  try {
    ...
    const { provider, sessionId, orderID } = parsed;
    ...
    const alreadyProcessed = await isOrderAlreadyProcessed(env, paymentId);
    if (alreadyProcessed) {
      console.log(`🚫 DUPLICATE DETECTED (${paymentId}) → SKIP`);
      return response(200, { success: true, message: 'Duplicate - already processed' });
    }

    const marked = await markOrderAsProcessed(env, paymentId);
    if (!marked) {
    ...
    if (provider === 'stripe') {
      ...
      if (!stripeRes.ok) throw new Error(`Stripe session fetch failed: ${await stripeRes.text()}`);
      session = await stripeRes.json();

      if (session.payment_status !== 'paid') throw new Error('Stripe not paid');

      const tempOrder = await getAndDeleteTempOrder(env, sessionId);
    ...
    // ====================== PAYPAL ======================
      ...
      const finalOrderRes  = await fetch(`${PAYPAL_BASE}/v2/checkout/orders/${orderID}`, {
        headers: { 'Authorization': `Bearer ${access_token}` }
      });
      ...
      if (cart.length === 0 && storedVariants.length > 0) {
        cart = storedVariants.map((str, i) => ({
          title: `Product ${i+1}`, price: 0, quantity: 1, variantsid: str || null, image: ''
        }));
      }
      ...
      shipping = {
        ...
        phone: payer.phone?.phone_number
          ? `+${payer.phone.phone_number.country_code || ''}${payer.phone.phone_number.national_number || ''}`
          : refParts[1] || '',
    ...
    const affRef = (provider === 'paypal'
      ? (purchaseUnit?.reference_id || '').split('|')[5]
      : shipping.affRef) || null;
